...
set<string> findLCS(string X, string Y, int m, int n)
{
    ...
    {
		// If LCS can be constructed from top side of the matrix
		// recurse for X[0..m-2] and Y[0..n-1]
        if (L[m - 1][n] >= L[m][n - 1])
            s = findLCS(X, Y, m - 1, n);
 
		// If LCS can be constructed from left side of the matrix 
		// recurse for X[0..m-1] and Y[0..n-2]
        if (L[m][n - 1] >= L[m - 1][n])
        {
