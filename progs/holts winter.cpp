 ...
 *   a[t] = a (Y[t] - s[t-p]) + (1-a) (a[t-1] + b[t-1])
 *   b[t] = ß (a[t] - a[t-1]) + (1-ß) b[t-1]
 *   s[t] = ? (Y[t] - a[t]) + (1-?) s[t-p]
 *
 * Multiplicative:
 *
...
int main() {
    ...
    double *estimated_level = (double*)malloc(nb_computations * sizeof(double));
    double *estimated_trend = (double*)malloc(nb_computations * sizeof(double));
    double *estimated_season = (double*)malloc(nb_computations * sizeof(double));
